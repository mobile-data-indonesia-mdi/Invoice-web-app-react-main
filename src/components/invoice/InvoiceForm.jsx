...
import invoiceSlice, { createNewInvoice, editInvoice, fetchAllInvoices} from '@/redux/invoiceSlice.js';
...
function InvoiceForm({ setOpenCreateInvoice, invoice, type }) {
  ...
  const [formData, setFormData] = useState({
    ...
    invoiceDate: type === "edit" ? moment(invoice.issue_date).format("YYYY-MM-DD") : "",
    dueDate: type === "edit" ? moment(invoice.due_date).format("YYYY-MM-DD") : "",
    ...
    items: type === "edit"
      ? invoice.invoice_details.map((detail, index) => ({
        ...
        total: (detail.delivery_count || 0) * (detail.price_per_delivery || 0),
        id: detail.invoice_detail_id || uuidv4(),
      }))
      : [{name: "", quantity: 1, usage: 0, price: 0, total: 0, id: uuidv4()}],
  })

  // Client State
  const clients = useSelector((state) => state.clients.allClients) || [];
  const [selectedClient, setSelectedClient] = useState(type === "edit" ? clients.find((c) => c.client_id === invoice.client_id) : {
    client_id: "",
    client_name: "",
    currency: "USD",
    client_address: "",
  ...
  const existingTaxInvoiceNumbers = otherInvoices.map((inv) => inv.tax_invoice_number);
  ...
  const onSubmit = async () => {
    if (existingInvoiceNumbers.includes(formData.invoiceNumber)) {
      alert(`Invoice number "${formData.invoiceNumber}" already exists. \nPlease use a different number.`);
      return;
    }

    if (existingTaxInvoiceNumbers.includes(formData.taxInvoiceNumber)) {
      alert(`Tax invoice number "${formData.taxInvoiceNumber || ''}" already exists.\nPlease use a different number.`);
      return;
    }
      ...
      >
        ...
        >
          ...
          <div className="overflow-y-scroll scrollbar-hide mb-8 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="col-span-3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const clientData = clients.find((client) => client.client_id === clientId);
                ...
                {selectedClient.client_id !== "" && (
                  <div className="mt-4 p-4 bg-gray-100 dark:bg-[#252945] rounded-lg shadow-md">
                    <h3 className="text-lg font-semibold">{selectedClient.client_name}</h3>
                    <table className="mt-4 border-collapse">
                      <tbody>
                      {[
                        { label: "Address", value: selectedClient.client_address },
                        { label: "Country", value: selectedClient.country },
                        { label: "Postal Code", value: selectedClient.postal_code },
                        { label: "Currency", value: selectedClient.currency },
                        { label: "Telephone", value: selectedClient.client_phone },
                      ].map((item, index) => (
                        <tr key={index}>
                          <td className="text-gray-500 dark:text-gray-400">{item.label}</td>
                          <td className="text-gray-500 dark:text-gray-400 px-2">:</td>
                          <td className="text-gray-800 dark:text-white">{item.value}</td>
                        </tr>
                      ))}
                      </tbody>
                    </table>
              ...
              {[{type: "text", label: "Invoice Number", name: "invoiceNumber", placeholder: "Enter invoice number"},
                ...
                <div key={index} className="col-span-3">
                  <label className="text-gray-400 dark:text-gray-300 block mb-1">{item.label}</label>
                  <input
                    type={item.type}
                    name={item.name}
                    ...
                    className={`w-full py-2 px-4 rounded border dark:bg-[#1E2139] dark:text-white ${
                      isValidatorActive && String(formData[item.name]).trim() === '' ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
                  />
                </div>
              ))}
              ...
              {formData.items.map((itemDetails) => (
                  ...
                  >
                    {[{type: "text", label: "Item Name", name: "name"},
                      {type: "number", label: "Quantity", name: "quantity", readOnly: true},
                      {type: "number", label: "Usage", name: "usage"},
                      {type: "number", label: `Price (${selectedClient.currency})`, name: "price"},
                      {type: "text", label: `Total (${selectedClient.currency})`, name: "total", readOnly: true}
                    ].map(
                      (item, index) => (
                        <div key={index} className="flex-1 mx-2">
                          <label className="block text-sm text-gray-500 dark:text-gray-400">{item.label}</label>
                          <input
                              type={item.type}
                              name={item.name}
                              value={item.name === "total" ? (itemDetails.usage * itemDetails.price).toFixed(2) : itemDetails[item.name]}
                              onChange={item.readOnly ? null : (e) => handleInputChange(e, itemDetails.id)}
                              className={`w-full py-2 px-4 mt-1 rounded border dark:bg-[#1E2139] dark:border-gray-600 dark:text-white ${
                                isValidatorActive && String(formData[item.name]).trim() === '' ? 'border-red-500' : 'border-gray-300'
                              }`}
            ...
            {[{type: "text", label: "Tax Invoice Number", name: "taxInvoiceNumber", placeholder: "Enter tax invoice number"},
              {type: "number", label: "Tax (%)", name: "tax", placeholder: "Enter tax percentage (e.g., 11)"},
            ].map((item, index) => (
                <div key={index} className="col-span-3">
                  <label className="text-gray-400 dark:text-gray-300 block mb-1">{item.label}</label>
                  <input
            ...
            <div className="text-right mt-6 space-y-2 text-sm">
              {[{label: "Subtotal", value: subtotal},
                ...
                <p key={index}>
                  <span className="font-bold">{item.label}:</span> {formatCurrency(
                    item.value,
